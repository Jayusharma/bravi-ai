...
**Bootstrap** (`main.ts`, `app.module.ts`) — Nest bootstrap, `api/v1` prefix, Redis IO adapter, global JWT guard, Swagger. *Partial:* no env validation, no graceful shutdown, no structured logging/Sentry, CaslGuard not global.

**auth** (`modules/auth/*`) — username/password login, bcrypt, JWT issue, `@Public()` bypass. *Partial:* the JWT signing secret and verifying secret are different env var names, both falling back to a hardcoded string (R3).

**user** (`modules/user/*`) — user CRUD, password change, deactivate. *Partial and insecure:* `POST /users` is `@Public()` and `role` is client-settable (R0); no CASL on any route.

**casl** (`modules/casl/*`) — builds CASL abilities from DB role→permission rows, supports `manage all` and a `$userId` placeholder (`casl-ability.factory.ts`). *Working as a mechanism, but not globally enforced* (R4).

**permission** (`modules/permission/*`) — CRUD for permissions, role→permission maps, subject bundles. *Partial and insecure:* the class-level `CaslGuard` is commented out (`permission.controller.ts:24`), so its `@CheckAbility` decorators are inert (R4).
...
**ingestion** (`modules/Ingestion/*`) — inbound → contact resolve → fast path (append to open enquiry) / reopen path / slow path (qualify). *Working,* but `POST /ingestion/message` is a second **public** injection route (R1), and inserts use `.create()` not `upsert` (`ingestion.service.ts:110,188,271`, R5).
...
**outbound** (`modules/outbound/*`) — validate → persist → queue → send → track. Includes the adapter interface + factory, Meta/Twilio/Email adapters, the channel router, the window service, delivery tracking, drafts + attachments, and the DLQ. *Partial:* the window service is never called (R2); the adapter interface has no capability flags (Block 2); Meta delivery receipts are dropped (R8).

**channels** (`modules/channels/*`) — ChannelConnection CRUD, the on/off toggle, and AES-256-GCM credential encrypt/decrypt (`common/crypto/credential-cipher.ts`), with live provider validation against SendGrid and Meta. *Working* for SendGrid + Meta; it stores no Meta `appSecret`, which inbound-Meta HMAC will need (R1).

**webhooks** (`modules/webhooks/*`) — inbound Twilio/SendGrid/Meta receive + normalize → ingest. Note `webhook.service.ts` is an **empty file**. *Partial:* no signature verification (R1); Meta status events are dropped (R8).
...
**websocket / events** — a single gateway holds all `@SubscribeMessage` handlers (`websocket/app.gateway.ts`); a single file holds all `@OnEvent` socket-emitting listeners (`events/app.event-handler.ts`). *Working,* convention holds. Leftover `console.log` at `app.gateway.ts:150-151` (R12); `outbound:send` and `contact:join` have no per-resource authorization.

**common / database** — idempotency (guard + interceptor + middleware), a response envelope, the global exception filter, the AES-GCM cipher, cursor paging, and the pg-adapter PrismaService. *Partial:* idempotency replay returns 409 (R5); the exception filter under-logs and leaks (R11).
...
**Block 1 — Trust & observability.** Mostly MISSING. No HMAC on inbound webhooks (`webhook.controller.ts:42,66,116`; `webhook.service.ts` empty; `main.ts:14` no raw body). Window check unwired (R2). No env validation (R9). No pino/cls, no Sentry, no `/health` or `/ready`, no graceful shutdown (R7). Idempotency is PARTIAL — dedup exists via the middleware but replay returns 409 (R5). The exception filter is PARTIAL (R11).
...
**R0 — Anyone on the internet can create an ADMIN account. CRITICAL. S.**
...
**R1 — No signature verification on inbound webhooks (billing DoS + fake enquiries). CRITICAL. M.**
...
**R2 — 24-hour WhatsApp window never enforced on send. HIGH. M.**
...
**R3 — JWT secret name mismatch plus a `dev-secret` fallback (token forgery). CRITICAL. S.**
...
**R4 — CASL is not global; PermissionController and UserController are unguarded. HIGH. M.**
...
**R5 — Idempotent webhook replay returns 409, not 200 (retry storms). HIGH. S.**
...
**R6 — The documented separate worker does not exist; workers run in-process. HIGH. M.**
...
**R7 — No graceful shutdown (deploys drop in-flight work). MEDIUM. S.**
...
**R8 — Meta delivery and read receipts are never recorded. MEDIUM. M.**
...
**R9 — No boot-time env validation. MEDIUM. S.**
...
**R10 — No rate limiting or helmet on public routes. MEDIUM. S.**
...
**R11 — Exception filter leaks internals and under-logs. MEDIUM. S.**
...
**R12 — Leftover debug and swallowed async rejections. LOW. S.**
...
**R13 — Rule-engine layers modeled but never executed. LOW/MEDIUM. M.**
...
**R14 — Status changes without a timeline event (audit gap). LOW. S.**
...
- Auth lockdown — remove `@Public()` from `POST /users`, drop client-settable `role`, add `@CheckAbility(create user)`, require the current password on change, register `CaslGuard` globally (or restore consistently) and add CASL to user/permission/ai/ingestion. (R0, R4)
- JWT secret unification — one `JWT_SECRET`, remove the `dev-secret` fallback in the strategy and the gateway. (R3)
- Env validation — Zod over the full env surface, fail boot on missing or malformed. (R9)
- Webhook trust — raw-body capture on webhook routes; `X-Hub-Signature-256`, `X-Twilio-Signature`, SendGrid signed key; store the Meta `appSecret`. (R1)
- Idempotency semantics — replay the stored 200 on a completed key; ingestion `.create()` → `upsert`. (R5)
- Window enforcement — call `isWindowOpen()` in the send path, typed rejection plus `contactId` log, template fallback hook. (R2)
- Observability — nestjs-pino + nestjs-cls (requestId, contactId, etc.), rework the exception filter (typed taxonomy, log every 5xx, no leak), Sentry (backend + frontend), `/health` and `/ready` (DB, Redis, adapter), graceful shutdown draining BullMQ and sockets. (R7, R11)
- Hardening — `@nestjs/throttler` and helmet on public routes; remove debug and swallowed catches. (R10, R12)
...
A capability-flagged `ChannelAdapter` plus connection-scoped routing, with no behaviour change, so Block 3 never touches the messaging core. Add a `capabilities` object (sending window, template-gated, subject, threading, rich attachments, read receipts, typing) to the interface and the adapters, a conformance test suite every adapter must pass, and a `channelConnectionId` on `ConversationMessage` (nullable, backfill null). Acceptance: each adapter declares its flags, the router uses `channelConnectionId` when present, the conformance suite is green for Meta/Twilio/Email, and outbound results are unchanged. Size: M.
